...
@section('title', 'Riwayat Pesananku - Cafe Ealnya')
...
<div class="mutasi-wrapper">
    ...
    @if($riwayat->isEmpty())
        <div style="text-align: center; padding: 50px;">
            ...
            <a href="{{ route('customer.pesanan.menu') }}" class="btn-custom" style="text-decoration: none; background: var(--pink-3); color: white; padding: 10px 20px; border-radius: 10px; display: inline-block;">Lihat Menu</a>
        </div>
    @else
        @foreach($riwayat as $trx)
            <div class="history-card" onclick="openDetail('{{ $trx->id }}')">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
                    <div>
                        <div style="font-size: 11px; color: #999;">{{ \Carbon\Carbon::parse($trx->tanggal)->format('d M Y, H:i') }}</div>
                        <div style="font-weight: 800; color: #333;">Order #TRX-{{ $trx->id }}</div>
                    </div>
                    <span class="status-badge {{ $trx->status == 'Menunggu Pembayaran' ? 'status-pending' : 'status-success' }}">
                        {{ $trx->status }}
                    </span>
                </div>

                <div style="font-size: 13px; color: #666;">
                    {{ $trx->pesanans->first()->nama_menu }} 
                    @if($trx->pesanans->count() > 1)
                        <span style="color: var(--pink-3);">+{{ $trx->pesanans->count() - 1 }} item lainnya</span>
                    @endif
                </div>
            ...
            <template id="data-{{ $trx->id }}">
                <div class="modal-body">
                    ...
                    <div class="detail-row">
                        ...
                        <span style="font-weight: 700; color: #333;">{{ $trx->metode_pembayaran ?? 'Cash' }}</span>
                    </div>
                    <div class="detail-row">
                        <span>Status</span>
                        <span class="status-badge {{ $trx->status == 'Menunggu Pembayaran' ? 'status-pending' : 'status-success' }}">{{ $trx->status }}</span>
                    </div>
                    ...
                    <div class="detail-row" style="font-size: 18px;">
                        ...
                        <span style="font-weight: 900; color: var(--pink-4);">Rp {{ number_format($trx->total_harga, 0, ',', '.') }}</span>
                    </div>
                </div>
            </template>
        @endforeach
