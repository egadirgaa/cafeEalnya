...
<body>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; max-width: 450px;">
            ...
            <div id="profile-trigger" onclick="openProfilePicker()" style="display: flex; align-items: center; gap: 10px; background: white; padding: 5px 12px; border-radius: 50px; border: 1px solid var(--pink-1); cursor: pointer;">
                <div style="text-align: right;">
                    ...
                    <p style="margin: 0; font-size: 12px; font-weight: 800; color: var(--pink-4);">{{ Auth::user()->name ?? 'Pelanggan' }}</p>
                </div>
                <img id="current-avatar" src="{{ asset('img_profile/avatar_1.jpeg') }}" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover; border: 2px solid white; box-shadow: 0 3px 10px rgba(0,0,0,0.1);">
            </div>
        </div>
    </header>
    ...
    <div id="profileModal" class="profile-modal" onclick="closeProfilePicker()">
        <div class="profile-card" onclick="event.stopPropagation()">
            ...
            <div class="avatar-grid">
                @for($i = 1; $i <= 10; $i++)
                    <img src="{{ asset('img_profile/avatar_'.$i.'.jpeg') }}" 
                         class="avatar-option" 
                         data-filename="avatar_{{ $i }}.jpeg"
                         onclick="selectAvatar('avatar_{{ $i }}.jpeg', this)">
                @endfor
            </div>
    ...
    <nav class="bottom-nav">
        <a href="{{ route('customer.pesanan.menu') }}" class="nav-item {{ request()->routeIs('customer.pesanan.menu') ? 'active' : '' }}">
            <span class="material-symbols-rounded">restaurant_menu</span>
            <p>Menu</p>
        </a>

        <a href="{{ route('customer.pesanan.index') }}" class="nav-item {{ request()->routeIs('customer.pesanan.index') ? 'active' : '' }}" id="cart-nav-item">
            <div style="position: relative;">
                ...
                @endphp
                <span id="cart-count" class="cart-badge" style="{{ $totalItem > 0 ? '' : 'display: none;' }}">{{ $totalItem }}</span>
            </div>
            <p>Keranjang</p>
        </a>

        @auth
            <a href="{{ route('customer.mutasi.index') }}" class="nav-item {{ request()->routeIs('customer.mutasi.index') ? 'active' : '' }}">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (savedAvatar) {
                avatarImg.src = `{{ asset('img_profile') }}/${savedAvatar}`;
            }
        });
        ...
        function selectAvatar(filename, element) {
            avatarImg.src = `{{ asset('img_profile') }}/${filename}`;
