...
<div class="mutasi-wrapper">
    <div class="header-section">
        ...
        <div class="count-pill">
            Total: <span id="totalCount">{{ $transaksis->count() }}</span>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table-eal">
                ...
                <tbody id="tableBody">
                    ...
                    <tr class="table-row" data-search="{{ strtolower($t->user->name ?? 'guest') }} #trx-{{ $t->id }}">
                        <td class="trx-id">#TRX-{{ str_pad($t->id, 4, '0', STR_PAD_LEFT) }}</td>
                        <td>{{ \Carbon\Carbon::parse($t->tanggal)->format('d M Y') }}</td>
                        <td class="customer-name">{{ $t->user->name ?? 'Guest User' }}</td>
                        <td class="total-price">Rp {{ number_format($t->total_harga, 0, ',', '.') }}</td>
                        <td>
                            <span class="status-badge {{ $t->status == 'selesai' ? 'status-done' : 'status-pending' }}">
                                {{ strtoupper($t->status) }}
                            </span>
                        </td>
                        <td style="text-align: center;">
                            <a href="{{ route('admin.mutasi.show', $t->id) }}" class="btn-detail">Detail</a>
                        </td>
                    </tr>
                    @empty
                    <tr id="noData">
