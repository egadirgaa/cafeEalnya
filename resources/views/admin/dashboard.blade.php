    ...
    <div class="stats-grid">
        <div class="stat-card-new reveal" id="card-1">
            ...
            <div>
                ...
                <h3 style="margin: 0; color: var(--text-main); font-size: 1.5rem;">
                    {{-- Tambahkan pengecekan jika null --}}
        ...
        <div class="stat-card-new reveal" id="card-2">
            ...
            <div>
                ...
                <h3 style="margin: 0; color: var(--text-main); font-size: 1.5rem;">{{ $transaksiHariIni->count() }} Order</h3>
            </div>
        </div>
        ...
        <div class="stat-card-new reveal" id="card-3">
            ...
            <div>
                ...
                <h3 style="margin: 0; color: var(--text-main); font-size: 1.5rem;">{{ $totalMenu }} Menu</h3>
            </div>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table style="width: 100%; border-collapse: separate; border-spacing: 0 12px;">
                ...
                <tbody id="dashTableBody">
                    @forelse($transaksiHariIni as $trx)
                    <tr class="dash-row">
                        <td style="padding: 15px 20px; background: #fffcfb; border-radius: 20px 0 0 20px;">
                            <div class="user-pill">
                                <div class="avatar-sm">{{ strtoupper(substr($trx->user->name ?? 'G', 0, 1)) }}</div>
                                <span>{{ $trx->user->name ?? 'Guest User' }}</span>
                            </div>
                        </td>
                        <td style="background: #fffcfb; color: #8d6e63; font-weight: 600;">{{ $trx->created_at->format('H:i') }} WIB</td>
                        <td style="background: #fffcfb; font-weight: 800; color: var(--secondary);">Rp {{ number_format($trx->total_harga, 0, ',', '.') }}</td>
                        <td style="background: #fffcfb;">
                            <span class="status-badge" style="{{ $trx->status == 'selesai' ? 'background:#e8f5e9;color:#2e7d32' : 'background:#fff3e0;color:#ef6c00' }}">
                                {{ $trx->status }}
                            </span>
                        </td>
                        <td style="background: #fffcfb; border-radius: 0 20px 20px 0; text-align: center;">
                            <a href="{{ route('admin.mutasi.show', $trx->id) }}" style="color: var(--primary);">
